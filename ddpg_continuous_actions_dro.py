...
class Args:
    ...
    # DRO Settings
    num_envs: int = 1
    """Number of parallel environments for each task"""
    task_probs_init: List[int] = None
    """Initial sampling distribution"""
    dro: bool = False
    """DRO on/off"""
    dro_num_steps: int = 8192
    """Frequency of DRO update"""
    dro_learning_rate: float = 0.1
    """Learning rate of DRO"""
    dro_eps: float = 0.1
    """Make the sampling distribution epsilon-greedy"""
    dro_success_ref: List[float] = None

    # Algorithm specific arguments
    env_ids: List[str] = field(default_factory=lambda: [f"PointMaze{i}-v0" for i in [1,4]])
...
"""
    ...
    for task_id in range(num_tasks):
        ...
        envs = gym.vector.SyncVectorEnv(
            [make_env(env_id=args.env_ids[task_id], seed=args.seed, idx=i, capture_video=args.capture_video, run_name=run_name)
    ...
    # Initialize Task Sampling Weights
    if args.task_probs_init is not None:
    ...
    for global_step in range(args.total_timesteps):
        ...
        # Sampling a task when an episode ends
        if "final_info" in infos:
            for info in infos["final_info"]:
        ...
        if args.dro and global_step % args.dro_num_steps == 0:
            ...
            if args.dro_success_ref is None:
                returns_ref = np.ones(1)
            else:
                returns_ref = args.dro_success_ref

            # Update the sampling probability
            task_probs = exponentiated_gradient_ascent_step(w=task_probs,
                                                            returns=training_returns_avg,
