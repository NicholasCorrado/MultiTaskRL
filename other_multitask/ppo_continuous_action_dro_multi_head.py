...
def simulate(env, actor, eval_episodes, eval_steps=np.inf, task_id=0):
    ...
    for episode_i in range(eval_episodes):
        ...
        while not done:
            ...
            with torch.no_grad():
                actions = actor.get_action(torch.Tensor(obs).to('cpu'), action=None, task_id=task_id)
...
class Agent(nn.Module):
    def __init__(self, envs, num_tasks, linear=False):
        ...
        # Multi-head log_std (one per task)
        self.actor_logstds = nn.ParameterList([
            nn.Parameter(torch.zeros(1, act_dim)) for _ in range(num_tasks)
        ])

    def get_value(self, x, task_id):
        features = self.shared_net(x)
        return self.critic_heads[task_id](features)

    def get_action_and_value(self, x, action=None, task_id=0):
        features = self.shared_net(x)
        action_mean = self.actor_mean_heads[task_id](features)
        action_logstd = self.actor_logstds[task_id].expand_as(action_mean)
        action_std = torch.exp(action_logstd)
        probs = Normal(action_mean, action_std)
        if action is None:
            action = probs.sample()
        return action, probs.log_prob(action).sum(1), probs.entropy().sum(1), self.critic_heads[task_id](features)

    def get_action(self, x, action=None, task_id=0):
        features = self.shared_net(x)
        action_mean = self.actor_mean_heads[task_id](features)
        action_logstd = self.actor_logstds[task_id].expand_as(action_mean)
        action_std = torch.exp(action_logstd)
...
if __name__ == "__main__":
    ...
    agent = Agent(envs, linear=args.linear, num_tasks=num_tasks).to(device)
    optimizer = optim.Adam(agent.parameters(), lr=args.learning_rate, eps=1e-5)

    # ALGO Logic: Storage setup
    list_obs = [torch.zeros((args.num_steps, args.num_envs) + envs.single_observation_space.shape).to(device) for i in range(num_tasks)]
    list_actions = [torch.zeros((args.num_steps, args.num_envs) + envs.single_action_space.shape).to(device) for i in range(num_tasks)]
    list_logprobs = [torch.zeros((args.num_steps, args.num_envs)).to(device) for i in range(num_tasks)]
    list_rewards = [torch.zeros((args.num_steps, args.num_envs)).to(device) for i in range(num_tasks)]
    list_dones = [torch.zeros((args.num_steps, args.num_envs)).to(device) for i in range(num_tasks)]
    list_values = [torch.zeros((args.num_steps, args.num_envs)).to(device) for i in range(num_tasks)]
    ...
    step = 0
    next_step = [0 for i in range(num_tasks)]
    ...
    returns_ref = [0 for i in range(num_tasks)]

    for global_step in range(1, args.num_iterations * args.num_steps + 1):
        if args.anneal_lr:
            frac = 1.0 - (global_step - 1.0) / (args.num_iterations * args.num_steps)
            lrnow = frac * args.learning_rate
            optimizer.param_groups[0]["lr"] = lrnow

        global_step += args.num_envs
        list_obs[task_id][step] = next_obs
        list_dones[task_id][step] = next_done

        # ALGO LOGIC: action logic
        with torch.no_grad():
            action, logprob, _, value = agent.get_action_and_value(next_obs, task_id=task_id)
            list_values[task_id][step] = value.flatten()
        list_actions[task_id][step] = action
        list_logprobs[task_id][step] = logprob

        # TRY NOT TO MODIFY: execute the game and log data.
        next_obs, reward, terminations, truncations, infos = envs.step(action.cpu().numpy())
        next_done = np.logical_or(terminations, truncations)
        list_rewards[task_id][step] = torch.tensor(reward).to(device).view(-1)
            ...
            for info in infos["final_info"]:
                if info and "episode" in info:
                    ...
                    if args.dro_success_ref:
                        ...
                        training_returns[task_id].append(np.mean(info["episode"]["r"]))
                    success_count[task_id] += info['is_success']
        ...
        if step == args.num_steps:
            obs = list_obs[task_id]
            dones = list_dones[task_id]
            values = list_values[task_id]
            actions = list_actions[task_id]
            logprobs = list_logprobs[task_id]
            rewards = list_rewards[task_id]
            ...
            for epoch in range(args.update_epochs):
                ...
                for start in range(0, args.batch_size, args.minibatch_size):
                    ...
                    _, newlogprob, entropy, newvalue = agent.get_action_and_value(b_obs[mb_inds], b_actions[mb_inds], task_id)
            ...
            update_count[task_id] += 1

            # print(task_probs)

        if global_step % (args.eval_freq * args.num_steps) == 0:
            print(f"Eval num_timesteps={global_step}")
            print(f'Task probs: {task_probs}')

            update_count_diff = [update_count[i] - last_update_count[i] for i in range(num_tasks)]
            print(f'Update numbers: {update_count}')
            print(f'Success count: {success_count}')
            total_update = int(np.sum(update_count_diff))
            update_count_diff = [update_count_diff[i] / total_update for i in range(num_tasks)]
            print(f'Update rates: {update_count_diff}')
            ...
            for j in range(num_tasks):
                ...
                return_avg, return_std, success_avg, success_std = simulate(env=envs_eval, actor=agent, eval_episodes=args.eval_episodes, task_id=j)
                ...
                logs[f'update_rate_{j}'].append(update_count_diff[j])
